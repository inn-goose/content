...
{{< alert "circle-info" >}}
The EEPROM chip type must be specified, as each chip has a distinct pin configuration that affects operation. The `--device` argument defines the correct chip type for the session.
{{< /alert >}}
